(() => {
  ...
  const DAY_EXTRAS = {
    1: { photo: "晋祠古建用24—35mm记录轴线与屋檐层次；植物园日落前拍温室建筑轮廓。", outfit: "太原9月底早晚偏凉：长袖/衬衫＋轻薄外套，白天热时可脱外层；舒适步行鞋，随身带防晒。", fallback: "航班或取车延误时优先保晋祠；植物园压缩为日落前短访。" },
    2: { photo: "山西博物院以建筑外观和晋魂重点展品为主；古城蓝调后再拍打铁花，手机开启连拍并避免过曝。", outfit: "太原→忻州，白天长袖或薄卫衣即可，古城夜间加防风外套；打铁花等待时间长，怕冷者带薄抓绒。", fallback: "打铁花临时取消时，改为古城夜景与小吃，不跨城追演出。" },
    3: { photo: "雁门关拍关城纵深和长城线条；木塔尽量用中长焦减少广角畸变。", outfit: "雁门关风口体感明显更低：长袖＋抓绒/薄毛衣＋防风外套，长裤、防滑步行鞋；早出可备薄羽绒。", fallback: "雁门关大风/管制时缩短为核心关城；净土寺自动取消。" },
    4: { photo: "悬空寺外观适合中长焦压缩山体与木构关系；浑源古建注意室内壁画禁闪光。", outfit: "悬空寺清晨山谷偏冷且风大，建议长袖打底＋抓绒＋防风外层；台阶多，穿防滑鞋，背包尽量轻。", fallback: "无登临票：寺下观景＋MR→永安寺→圆觉寺→大同。" },
    5: { photo: "云冈洞窟遵守禁拍区域；古城傍晚利用城墙和屋檐形成层次。", outfit: "大同全天步行：长袖＋轻外套，洞窟内外有温差；软底步行鞋，傍晚古城可加薄抓绒。", fallback: "云冈客流过大时压缩古城小景点，保云冈＋华严寺。" },
    6: { photo: "乌兰哈达用广角拍火山锥与公路尺度，风沙天保护镜头。", outfit: "进入内蒙古后风更明显：长袖＋抓绒/卫衣＋防风外套，帽子固定好；火山无遮挡，墨镜和SPF50+防晒随身。", fallback: "大风/沙尘/降雨时只保1个核心火山点后直去呼和浩特。" },
    7: { photo: "翻大青山时只在正规停车点记录山南—山北地貌变化；草原日落用低机位拍草甸层次。", outfit: "希拉穆仁草原昼夜温差最大的一天之一：白天长袖＋防风层，日落后增加抓绒或薄羽绒；帽子、围巾/头巾可兼顾保暖防风。", fallback: "草原天气差时缩短户外，优先博物院与营地休息。" },
    8: { photo: "长转场日不追景，包头/河套沿线只做安全短停。", outfit: "长途车内外温差大，采用洋葱式穿搭：长袖打底＋薄抓绒＋可收纳防风外套；车内备薄羽绒，方便服务区临时加衣。", fallback: "国庆首日拥堵明显时取消包头额外停留，只补给。" },
    9: { photo: "岩画如正规开放，使用中长焦记录纹样，不触摸岩面；腾格里第一晚优先沙丘日落。", outfit: "阴山沟谷＋腾格里沙漠都偏干且有风：长袖长裤、防风外套、墨镜、帽/头巾；傍晚明显降温，薄羽绒放车上随取。", fallback: "阴山岩画道路/接待任一不确认就取消；乌海湖只做可选短停。" },
    10:{ photo: "上午拍湖色、沙丘线条与车辙，中午减少暴晒，17:00后拍日落蓝调。", outfit: "沙漠日晒与风沙同时存在：速干长袖长裤＋防风外层，墨镜、宽檐帽/头巾、SPF50+；夜间观星加抓绒或薄羽绒。", fallback: "大风、沙尘、强降雨或运营方判断不宜深入时缩短线路，不以集齐五湖为目标。" },
    11:{ photo: "66号公路只在安全停车区域构图，绝不站车道中央；黄河沿线可用长焦拍层次。", outfit: "进入宁夏后仍以干燥和风为主：长袖＋轻防风外套，白天可减层；公路拍摄备墨镜、防晒和润唇膏。", fallback: "体力不足时取消北长滩/黄河沿线，只保高庙＋66号公路。" },
    12:{ photo: "览山公园日落前就位，拍贺兰山方向日落和蓝调；夜市用手机夜景模式。", outfit: "中卫→吴忠→银川转场：白天长袖/薄卫衣，傍晚览山开阔地降温快，增加防风外套；怕冷者备轻薄羽绒。", fallback: "览山如因活动临时封闭，直接银川市区晚餐休息。" },
    13:{ photo: "中卫高庙与沙坡头兼顾古建、黄河和沙丘；66号公路只在合法停车位取景。", outfit: "中卫沙坡头日晒强、沙粒细：速干长袖长裤＋防风外套，墨镜、头巾、SPF50+；鞋口尽量贴合，傍晚加薄抓绒。", fallback: "当天优先级：高庙→沙坡头 > 66号公路；时间紧则压缩66号公路。" },
    14:{ photo: "宁夏博物馆以展陈记录为主，览山日落和怀远夜市作为轻松收尾。", outfit: "银川城市日：长袖＋轻外套即可，览山日落前后加防风层；夜市步行多，穿舒适鞋并注意保湿。", fallback: "中卫返银川因车流延误时，博物馆只看历史＋西夏主线，不挤占返程和休息。" },
    15:{ photo: "返程日不安排追光，把精力留给还车、行李和机场流程。", outfit: "返程以舒适和方便安检为主：长袖/薄卫衣＋可脱外套，运动鞋；车上保留一件防风层直到还车。", fallback: "提前检查油量、车况照片、证件和随身行李；航班时间变化时以还车和登机缓冲优先。" }
  };
  const TICKETS = [
